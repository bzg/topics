...
: :theme   - Theme name to load from CDN (e.g. swh, dsfr, org or doric)

Use =$ topics -c my_config.edn input.json= to use another file name than
=config.edn=.

The =index.html= output can link to a theme CSS loaded from
=https://cdn.jsdelivr.net/gh/bzg/pico-themes@main/{name}.css=.

#+begin_src sh
$ topics -t swh input.json
#+end_src
